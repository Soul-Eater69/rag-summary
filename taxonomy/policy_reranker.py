    ...
    *,
    ...
    for entry in working:
        ...
        entry["eligibility_score"] = round(
            max(0.0, entry["fused_score"] + adj), 3
        )
    ...
    for entry in working:
        ...
        # Also check if it was promoted as downstream — that bypasses semantic suppression
        bucket = entry["bucket"]
        is_promoted = "downstream_promotion" in entry.get("rationale", "")

        if positive_matches < _SIGNAL_CONFIRM_MIN and not is_promoted:
